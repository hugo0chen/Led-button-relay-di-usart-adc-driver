...
//SPI显示屏接口
//LCD_RST
#define SPILCD_RST_SET  GPIO_SetBits(GPIOB, GPIO_Pin_6)//PB0 
#define SPILCD_RST_RESET GPIO_ResetBits(GPIOB, GPIO_Pin_6)//PB0  
//LCD_RS//dc  
#define SPILCD_RS_SET  GPIO_SetBits(GPIOB, GPIO_Pin_7)//PC4 
#define SPILCD_RS_RESET  GPIO_ResetBits(GPIOB, GPIO_Pin_7)//PC4  
//LCD_CS  
#define SPILCD_CS_SET GPIO_SetBits(GPIOB, GPIO_Pin_2)//PA4 
#define SPILCD_CS_RESET  GPIO_ResetBits(GPIOB, GPIO_Pin_2)//PA4 
...
void LCD_ShowString(u16 x, u16 y, u16 width, u16 height, u8 size,
		const char *p);		//显示一个字符串,12/16字体

void LCD_WriteReg(u8 LCD_Reg, u16 LCD_RegValue);
void LCD_WriteRAM_Prepare(void);
void LCD_WR_DATA8(u8 da);   //写8位数据  

void refshow(char state);	 //刷新显示;	 //刷新显示
void showqq(void);
